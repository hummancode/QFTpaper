function [] = plot_NicholsVel2(gmin,gmax,EHAphase,w_a,Pnom,Gcont)
am=cell2mat(gmin);
bm=cell2mat(gmax);
...
h=plot(c,20*log10(b),'Color',mark(mod(count-1,6)+1));
...
hold on
h=plot(c,20*log10(a),'--','Color',mark(mod(count-1,6)+1));
h.Annotation.LegendInformation.IconDisplayStyle = 'off';
...
plot(pphase,20*log10(p),'o','Color',mark(mod(count-1,6)+1),'DisplayName',strcat('w=',num2str(w_a(count)),' rad/s'),LineWidth=3);
...
for i=1:length(wspan)
    ...
    plot(cell2mat(pparray),20*log10(cell2mat(parray)),'Color','k','LineWidth',2,'DisplayName','L(s)');
    XLabel.String = '[degree]';
    YLabel.String = '[dB]';
